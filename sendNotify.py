...
if "SCKEY" in config['send'][0] and config['send'][0]["SCKEY"]:
    SCKEY = config['send'][0]["SCKEY"]
if "TG_BOT_TOKEN" in config['send'][0] and config['send'][0]["TG_BOT_TOKEN"] and "TG_USER_ID" in config['send'][0] and config['send'][0]["TG_USER_ID"]:
    TG_BOT_TOKEN = config['send'][0]["TG_BOT_TOKEN"]
    TG_USER_ID = config['send'][0]["TG_USER_ID"]
if "TG_API_HOST" in config['send'][0] and config['send'][0]["TG_API_HOST"]:
    TG_API_HOST = config['send'][0]["TG_API_HOST"]
if "DD_BOT_ACCESS_TOKEN" in config['send'][0] and config['send'][0]["DD_BOT_ACCESS_TOKEN"] and "DD_BOT_SECRET" in config['send'][0] and config['send'][0]["DD_BOT_SECRET"]:
    DD_BOT_ACCESS_TOKEN = config['send'][0]["DD_BOT_ACCESS_TOKEN"]
    DD_BOT_SECRET = config['send'][0]["DD_BOT_SECRET"]
if "QQ_SKEY" in config['send'][0] and config['send'][0]["QQ_SKEY"] and "QQ_MODE" in config['send'][0] and config['send'][0]["QQ_MODE"]:
    QQ_SKEY = config['send'][0]["QQ_SKEY"]
    QQ_MODE = config['send'][0]["QQ_MODE"]
# 获取pushplus+ PUSH_PLUS_TOKEN
if "PUSH_PLUS_TOKEN" in config['send'][0]:
    if len(config['send'][0]["PUSH_PLUS_TOKEN"]) > 1:
        PUSH_PLUS_TOKEN = config['send'][0]["PUSH_PLUS_TOKEN"]
        # print("已获取并使用Env环境 PUSH_PLUS_TOKEN")
# 获取企业微信应用推送 QYWX_AM
if "QYWX_AM" in config['send'][0]:
    if len(config['send'][0]["QYWX_AM"]) > 1:
        QYWX_AM = config['send'][0]["QYWX_AM"]
